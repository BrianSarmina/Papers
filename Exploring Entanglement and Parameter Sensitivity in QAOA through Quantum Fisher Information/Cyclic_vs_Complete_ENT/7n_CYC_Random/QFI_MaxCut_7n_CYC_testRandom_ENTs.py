...
def createMaxCut_qiskit(qc, q_num, gammas, betas1, combs):
...
for sol in data:
    ...
    parameters = [gammas, betas1]
    # Parameter-shift value
    s = np.pi/2

    qc = QuantumCircuit(q_num)
    qc = createMaxCut_qiskit(qc, q_num, gammas, betas1, combs)
##    break
    # Initialize QFI.
    estimator = Estimator()
